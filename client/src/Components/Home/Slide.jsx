...
const MultiSlide = ({ data = [], timer, title }) => {
  ...
  const renderer = ({ hours, minutes, seconds }) => (
    <RenderTimer variant="span">{hours} : {minutes} : {seconds} Left</RenderTimer>
  );

  if (!Array.isArray(data) || data.length === 0) {
    return null; // ✅ évite le crash si data est vide ou non chargé
  }
  ...
  return (
    <Component>
      ...
      >
        {data.map((temp, index) => (
          <Link to={`product/${temp.id}`} key={temp.id || index} style={{ textDecoration: 'none' }}>
            <Box textAlign="center" style={{ padding: '25px 15px' }}>
              <Image src={temp.url} alt={temp.title?.shortTitle || 'Product'} />
